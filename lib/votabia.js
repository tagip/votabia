...
const DAYENDMIN = 0;
const LUNCHSTARTHOUR = process.env.LUNCHSTARTHOUR || 12;
const LUNCHENDHOUR = process.env.LUNCHENDHOUR || 14;
...
class Votabia {
  ...
  isWorkingHour() {
    ...
    if (now.getHours() > LUNCHSTARTHOUR && now.getHours() < LUNCHENDHOUR) {
      return false;
    }
    if (now.getHours() == LUNCHSTARTHOUR && now.getMinutes() > LUNCHSTARTMIN) {
      return false;
    }
    if (now.getHours() == LUNCHENDHOUR && now.getMinutes() < LUNCHENDMIN) {
      return false;
    }
  ...
  checkReset() {
    var now = new Date();
    if (now.getHours() == DAYSTARTHOUR && now.getMinutes() == DAYSTARTMIN && now.getSeconds() == 0 || now.getHours() == LUNCHENDHOUR && now.getMinutes() == LUNCHENDMIN && now.getSeconds() == 0) {
